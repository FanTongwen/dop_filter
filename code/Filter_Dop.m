...
%         end
    ...
    if (Indexflag == 1)&&(obs{Index}.SNR > 32.0)%确保有星且载噪比合适
        ...
        if (sum(PRN == obs{Index}.sat)) == 0
            ...
            info_dopfilter = [info_dopfilter, InfoSave];
        elseif (sum(PRN == obs{Index}.sat)) == 1
            ...
            info_dopfilter{PRN == obs{Index}.sat} = [info_dopfilter{PRN == obs{Index}.sat}, InfoSave];
        end
        % save ref_sat info
        ...
        for i = 1:min(n, 64)
            ...
            % 保存其他信息
            InfoSave.diff = Star_INS_DDiff;
            InfoSave.diff_s = Star_INS_DDiff_s;
            InfoSave.gerror = Star_INS_DDiff - Star_INS_DDiff_s;
            % 保存修正的多普勒信息
            if (sum(PRN == obs{i}.sat)) == 0
                ...
                predict_dop_p = [predict_dop_p, obs{i}.Pack_D];%预测多普勒保存
                info_dopfilter = [info_dopfilter, InfoSave];
            elseif (sum(PRN == obs{i}.sat)) == 1
                ...
                info_dopfilter{PRN == obs{i}.sat} = [info_dopfilter{PRN == obs{i}.sat}, InfoSave];
            end
        end
    else %换星
        ...
        if RefSate ~= RefPrn
            if RefSate > 0 %换星成功
                ...
                elseif (sum(PRN == obs{Chooseindex}.sat)) == 1
                    ...
                    info_dopfilter{PRN == obs{Chooseindex}.sat} = [info_dopfilter{PRN == obs{Chooseindex}.sat}, InfoSave];
                end
...
function Filter_Result =  MeasureFilter_DoplorFunc1(satenum, temDop)
    global SateMap;
    global MeasureData;
    global MeasureSate_Count;
    temdoulist = [];
